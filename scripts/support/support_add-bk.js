...
function add(){
  ...
  if(h.vat_type == '' || h.tax_status == '') {
    swal("กรุณาเลือกชนิด VAT");
    $('#vat-type').addClass('has-error').focus();
    click = 0;
    return false;
  }
  ...
  if(h.customer_code.length == 0) {
    ...
    $('#customer-code').addClass('has-error').focus();
    click = 0;
    return false;
  }
  ...
  if(h.customer_name.length == 0){
    ...
    $('#customer-name').addClass('has-error').focus();
    click = 0;
    return false;
  }
  ...
  if( ! isDate(h.date_add))
  {
    ...
    $('#date').addClass('has-error').focus();
    click = 0;
    return false;
  }  

  if(h.warehouse.length == 0){
    swal('กรุณาเลือกคลัง');
    $('#warehouse').addClass('has-error').focus();
    click = 0;
    return false;
  }
