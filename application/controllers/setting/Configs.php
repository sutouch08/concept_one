...
class Configs extends PS_Controller
{
  ...
  public function index($tab = 'company')
  {
    ...
    //--- select credit card
    $cd = $this->ms->select('CreditCard, CardName, AcctCode')->get('OCRC');
    $ac = $this->ms->select('AcctCode, AcctName')->get('OACT');

    $ds['acc_code_select'] = $ac->num_rows() > 0 ? $ac->result() : NULL;
    $ds['credit_card_select'] = $cd->num_rows() > 0 ? $cd->result() : NULL;
    $ds['cando'] = $cando;
    $ds['tab'] = $tab;
